    ...
    return (
        <div className="w-full px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8 pb-28 pt-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center space-x-2.5 bg-white dark:bg-gray-800/80 px-3.5 py-2 rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-white/10 shrink-0">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-center justify-between bg-white dark:bg-gray-800/80 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-white/10">
                ...
                <div className="flex space-x-2.5 w-full sm:w-auto">
                    <button
                        ...
                        className="flex-1 sm:flex-none px-4 sm:px-5 py-2.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all border border-emerald-100 dark:border-emerald-900/40"
                    >
                        Mark All Present
                    </button>
                    <button
                        onClick={() => handleMarkAll('ABSENT')}
                        className="flex-1 sm:flex-none px-4 sm:px-5 py-2.5 bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all border border-rose-100 dark:border-rose-900/40"
                    >
                        Mark All Absent
                    </button>
                </div>
                ...
                <AnimatePresence mode="popLayout">
                    {filteredStudents.length > 0 ? filteredStudents.map((s, idx) => (
                        <motion.div
                            ...
                            layout
                        >
                            <div className="bg-white dark:bg-gray-800/80 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors hover:border-indigo-500/30 dark:hover:border-indigo-400/30">
                                ...
                                <div className="flex items-center justify-between sm:justify-end bg-gray-50 dark:bg-gray-900/60 p-1.5 rounded-xl sm:rounded-2xl border border-gray-200/60 dark:border-gray-700/60 gap-1">
                                    <button
                                        ...
                                        className={`flex-1 sm:flex-none px-4 sm:px-6 py-2 rounded-lg sm:rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${attendance[s.id] === 'PRESENT' ? 'bg-emerald-600 text-white' : 'text-gray-400 hover:text-emerald-600'}`}
                                    >
                                        Present
                                    </button>
                                    <button
                                        onClick={() => handleStatusChange(s.id, 'LATE')}
                                        className={`flex-1 sm:flex-none px-4 sm:px-6 py-2 rounded-lg sm:rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${attendance[s.id] === 'LATE' ? 'bg-amber-500 text-white' : 'text-gray-400 hover:text-amber-500'}`}
                                    >
                                        Late
                                    </button>
                                    <button
                                        onClick={() => handleStatusChange(s.id, 'ABSENT')}
                                        className={`flex-1 sm:flex-none px-4 sm:px-6 py-2 rounded-lg sm:rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${attendance[s.id] === 'ABSENT' ? 'bg-rose-600 text-white' : 'text-gray-400 hover:text-rose-600'}`}
                                    >
                                        Absent
                                    </button>
                                </div>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-16 text-center bg-slate-50/50 dark:bg-white/[0.02] rounded-2xl sm:rounded-3xl border border-dashed border-slate-200/80 dark:border-white/10">
            ...
            >
                <div className="bg-[#2D2A4F]/95 dark:bg-gray-900/95 backdrop-blur-xl px-5 sm:px-8 py-3.5 sm:py-4 rounded-2xl sm:rounded-3xl flex items-center justify-between space-x-4 sm:space-x-8 pointer-events-auto border border-white/10">
                    ...
                    <Button
                        onClick={handleSave}
                        isLoading={saving}
                        className="bg-white text-gray-900 hover:bg-gray-100 border-none px-6 sm:px-10 py-2.5 rounded-xl font-black uppercase text-xs tracking-widest flex items-center"
                    >
